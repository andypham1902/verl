...
def main():
    ...
    for j in range(0, len(df), 8):
        ...
        while i <= 1:
            ...
            i += 1
        try:
            if os.path.exists("_llama70b_50k.parquet"):
                existing_df = pd.read_parquet("_llama70b_50k.parquet")
                combined_df = pd.concat([existing_df, _df], ignore_index=True)
                combined_df.to_parquet("_llama70b_50k.parquet", index=False)
            else:
                _df.to_parquet("_llama70b_50k.parquet", index=False)
        except Exception as e:
